...
public class OfferServiceImpl implements OfferService {
	...
	/*
	 * @Scheduled(fixedRate = OFFER_SCHEDULED_FIX_RATE) private void
	 * generateOffers() { this.offerRepository.deleteAll();
	 * List<ProductServiceModel> products = this.productService
	 * .findAllProducts().stream() .filter(p->!p.isDeleted())
	 * .filter(p->p.getCategories().stream().anyMatch(c->!c.isDeleted()))
	 * .collect(Collectors.toList());;
	 * 
	 * if (products.isEmpty()) { return; }
	 * 
	 * int n = products.size() > OFFER_SCHEDULED_NUMBER_OF_PRODUCTS?
	 * OFFER_SCHEDULED_NUMBER_OF_PRODUCTS : products.size();
	 * 
	 * Random rnd = new Random(); List<Offer> offers = new ArrayList<>(); for (int i
	 * = ZERO_NUMBER; i < n; i++) { Offer offer = new Offer();
	 * offer.setProduct(this.modelMapper.map(products.get(rnd.nextInt(products.size(
	 * ))), Product.class));
	 * offer.setPrice(offer.getProduct().getPrice().multiply(new
	 * BigDecimal(OFFER_SCHEDULED_DISCOUNT)));
	 * 
	 * if (offers.stream().filter(o ->
	 * o.getProduct().getId().equals(offer.getProduct().getId())).count() ==
	 * ZERO_NUMBER) { offers.add(offer); } }
	 * 
